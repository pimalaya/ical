...
**The un-indexed setters replace the whole value.** Keeping them on component zero while their readers read the whole value would break the identity round trip: `text()` on `DESCRIPTION:a;b` reading `a;b` and `set_text("a;b")` writing only component zero leaves `DESCRIPTION:a\;b;b`. A reader and a writer at different scopes is a data-loss generator, so they were moved to the same scope. `set_component` still rewrites nothing but the component it names, which is what byte preservation rests on.

**A single-valued component of a structured value reads joined, not scalar.** A `REQUEST-STATUS` description and its extra data are texts, where a comma separates nothing. The deleted `decode_scalar_at` was cutting them there.

**`RECUR` reads the whole value.** Its rule parts are `;`-separated, so the codec was walking the components and rejoining them by hand. That is what the new whole-value read does, so the hand-rolled loop is gone and the borrow it used to lose is kept.

**An emptied list holds no items.** The wire cannot tell an empty list from a list of one empty item, so the whole-value list read answers one empty item for an emptied value, where the component read answered none. The merge's list replay reads its own writes, so it now treats an all-empty list as empty rather than writing a leading comma into the next addition.
