...
//! `DTEND` is a duration, an `ATTENDEE` line is a Participant object, a
//! `VALARM` is an Alert, and an overriding `VEVENT` is not a component at all
//! but a patch inside the series it overrides. The conversion rules are those
//! of
...
//! `convertedProperties` record (draft 5.1.1). The same record names the
//! property a member came from wherever more than one could have, so `updated`
//! knows whether it was a `DTSTAMP` or a `LAST-MODIFIED`.
//!
//! Importing, the mirror hatch applies: a member with no iCalendar counterpart
//! becomes a `JSPROP` property holding its JSON, located by a `JSPTR`
//! parameter (draft 4.1.2, 4.2.2), and a collection key that was not simply
//! the element's position is carried on a `JSID` parameter so it survives the
//! next conversion.
//!
//! ## What normalises
//!
...
//! time-zone database, which this crate does not carry, so the wall-clock
//! digits are carried across unshifted: exact for a floating or UTC object,
//! and off by that zone's offset for any other. The whole of
//! [`timezone`](crate::timezone) is available to a caller that wants to shift
//! it from the calendar's own `VTIMEZONE`.
//!
