...
/// A single raw text piece of a calendar (a name, a parameter value, a line
...
#[derive(Clone, Debug)]
pub struct IcalLeaf<'a>(pub Cow<'a, str>);
...
impl<'a> IcalLeaf<'a> {
    ...
    pub(crate) fn into_static(self) -> IcalLeaf<'static> {
        IcalLeaf(Cow::Owned(self.0.into_owned()))
    }
}

impl<'a> From<&'a str> for IcalLeaf<'a> {
    fn from(text: &'a str) -> Self {
        Self(Cow::Borrowed(text))
    }
}

impl From<String> for IcalLeaf<'_> {
    fn from(text: String) -> Self {
        Self(Cow::Owned(text))
    }
}

/// A single raw *value* component of a calendar, held as raw bytes so a foreign
...
#[derive(Clone, Debug)]
pub struct IcalValueLeaf<'a>(pub Cow<'a, [u8]>);
...
impl<'a> IcalValueLeaf<'a> {
    ...
    pub(crate) fn into_static(self) -> IcalValueLeaf<'static> {
        IcalValueLeaf(Cow::Owned(self.0.into_owned()))
    }
}
...
impl From<Vec<u8>> for IcalValueLeaf<'_> {
    fn from(bytes: Vec<u8>) -> Self {
        Self(Cow::Owned(bytes))
    }
}

impl<'a> From<Cow<'a, str>> for IcalValueLeaf<'a> {
...
mod tests {
    ...
    use crate::tree::leaf::{IcalLeaf, IcalValueLeaf};

    #[test]
    fn replaces_leaf_contents() {
        let mut text = IcalLeaf::from("a");
        text.set("b");
        assert_eq!(text.get(), "b");

        let mut bytes = IcalValueLeaf::from(b"a".as_slice());
