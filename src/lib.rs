...
//! sits behind a [cargo feature](#cargo-features).
//!
...
//! [`validate`](tree::ical::validate) checks a decoded calendar against its
...
//! property is an [`IcalProp`](prop::IcalProp) of a name, parameters and one
//! value, the last two open payload enums ([`IcalParam`](param::IcalParam),
//! [`IcalValue`](value::IcalValue)) with an `Unknown` arm, so anything outside
//! the model survives.
//!
//! The syntax tree ([`tree`], behind the default `parser` feature) is
//! everything byte-faithful. Its hub is [`IcalCst`](tree::cst::IcalCst), a
...
//!
//! [`parse`](tree::cst::IcalCst::parse) reads one calendar and
//! [`parse_many`](tree::cst::IcalCst::parse_many) iterates a multi-calendar
//! file. Both are strict and refuse a calendar they cannot structure, while
//! [`parse_recovering`](tree::cst::IcalCst::parse_recovering) keeps what it
...
//! whole. [`decode`](tree::cst::IcalCst::decode) projects a CST onto the decoded
//! [`Ical`](ical::Ical), and `encode` (with `From<Ical>`) projects the model
//! back to a canonical CST. Per-property and per-component lens markers
//! ([`IcalPropLens`](tree::prop::lens::IcalPropLens),
//! [`IcalComponentLens`](tree::component::lens::IcalComponentLens)) read or edit one
//! line or subtree through the byte-preserving
//! [`cursor`](tree::value::cursor::IcalValueCursor)s, so editing one property leaves
//! every other byte intact.
//!
//! ## The spec layer
//!
//! Each property carries an [`IcalPropSpec`](tree::prop::spec::IcalPropSpec) on its
...
//! [`IcalComponentSpec`](tree::component::spec::IcalComponentSpec) (the children it
...
//! [`validate`](tree::ical::validate) and the builder all consult one source of
//! truth. A calendar that passes earns an [`IcalValid`](valid::IcalValid) proof,
...
//!
//! [`merge`](tree::merge) is the syntax layer's answer to two divergent edits of
//! one calendar: [`IcalMerge`](tree::merge::IcalMerge) diffs each against their
...
//!   `serde_json` crate. Implies `parser` for the property specs.
//! - `jscalendar`: the RFC 8984 JSON data model of a calendar. Implies `jcal`,
//!   whose syntax carries the escape hatch, and pulls no crate of its own.
